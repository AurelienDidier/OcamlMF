...
let parse (s : string) : expr =
  ...
  let format = Parser.prog Lexer.read lexbuf in
  format
...
let pair_err = "Pairs must have same type"
...
and typeof_pair e1 e2 =
  let t1, t2 = typeof Context.empty e1,typeof Context.empty e2 in
  match  t1, t2 with
  | TInt, TInt -> TInt
  | _ -> failwith pair_err
